...
f32 Lighting::CalculateAO(const BVH& bvh, const Vector3& position, const Vector3& normal) const
{
    f32 occlusion   = 0.0f;
    f32 weight      = 1.0f;
    Vector3 stepVec = normal * AOStepSize;
    Vector3 p       = position;
    ...
    for (s32 i = 1; i <= AOSteps; ++i)
    {
        ...
        occlusion += (i * AOStepSize - sdfDist) * weight;
        weight    *= 0.5f;
    }
...
Vector3 Lighting::Shade(const BVH& bvh, const SurfacePoint& surface, const Vector3& viewDir) const
{
    ...
    f32 oneMinusK = 1.0f - k;
    f32 nDotV     = std::max(surface.Normal.Dot(viewDir), 0.0f);
    f32 gV        = nDotV / (nDotV * oneMinusK + k);
    ...
    for (const Light& light : _lights)
    {
        ...
        if (isPointLight)
        {
            ...
            attenuation  = light.Intensity / distanceSquared;
        }
        else
        {
        ...
        bool occluded        = bvh.IntersectAny(Ray(shadowOrigin, lightDir), 0.0001f, maxT);
        ...
        f32 gL = nDotL / (nDotL * oneMinusK + k);
        f32 G  = gV * gL;

        f32 oneMinusVDotH  = 1.0f - vDotH;
        f32 oneMinusVDotH2 = oneMinusVDotH  * oneMinusVDotH;
        f32 fresnelWeight  = oneMinusVDotH2 * oneMinusVDotH2 * oneMinusVDotH;
        Vector3 F          = f0 + (Vector3(1.0f) - f0) * fresnelWeight;
        ...
        Vector3 radiance = light.Color * (isPointLight ? attenuation : light.Intensity);
